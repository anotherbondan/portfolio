...
export default function Project({ data, children }: ProjectProps) {
  return (
    <Card className="group relative flex w-full flex-col gap-6 rounded-3xl border border-black/5 bg-white/70 p-5 transition-all duration-500 before:absolute before:-inset-4 before:-z-10 before:content-[''] hover:-translate-y-2 hover:border-primary-500/30 lg:flex-row lg:gap-8 lg:rounded-[2rem] lg:p-8 dark:border-white/5 dark:bg-neutral-900/70">
      {/* Background Glow on hover */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden rounded-3xl bg-gradient-to-br from-primary-500/5 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100 lg:rounded-[2rem]" />
      ...
      <div className="z-10 flex w-full flex-col justify-between py-1 lg:w-3/5 lg:py-2">
        <div className="flex flex-col gap-4 lg:gap-2">
          <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
            <h1 className="font-playfair-display text-2xl font-bold text-neutral-800 transition-colors duration-300 group-hover:text-primary-600 lg:text-3xl dark:text-neutral-200 dark:group-hover:text-primary-300">
              {data.title}
            </h1>
            <span className="font-inter self-start rounded-full border border-primary-500/20 bg-primary-500/10 px-3 py-1.5 text-[10px] font-semibold tracking-wide text-primary-600 lg:text-xs dark:text-primary-300">
              {data.date}
            </span>
          </div>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:gap-4 lg:mt-8">
          {data.projectUrl && (
            ...
            >
              <Button className="font-inter flex w-full items-center justify-center gap-2 rounded-xl bg-primary-600 px-4 py-3 text-sm font-semibold text-white shadow-lg transition-all duration-300 hover:bg-primary-500 lg:px-6">
                View Project
